...
from servizioaggiuntivo.AggiungiServizioAggiuntivo import AggiungiServizioAggiuntivo
from servizioaggiuntivo.ModificaServizioAggiuntivo import ModificaServizioAggiuntivo


class ServizioAggiuntivo(QDialog):
    def __init__(self):
        super(ServizioAggiuntivo, self).__init__()
        loadUi("servizioaggiuntivo/servizioaggiuntivo2.ui",self)
        self.tableWidget.setEditTriggers(self.tableWidget.NoEditTriggers)
        self.tableWidget.setHorizontalHeaderLabels(["ServizioAggiuntivo", "Prezzo"])
    ...
    def goto_aggiungi(self):
        add = AggiungiServizioAggiuntivo()
    ...
    def goto_modifica(self):
        try:
            ...
            self.mod = ModificaServizioAggiuntivo(self.selected)
    ...
    def popola_lineEdit(self):
            ...
            self.mod.tipo.setText(str(result[self.selected][0]))
            self.mod.prezzo.setText(str(result[self.selected][1]))
    ...
    def funz_cancella(self):
        try:
            ...
            if reply == QMessageBox.Yes:
                ...
                prezzi_sel = self.prezzi[self.selected]

                query2 = "DELETE FROM `servizioaggiuntivo` WHERE `servizioaggiuntivo`.`Tipo` = '" + codiceSel.__str__() + "' AND `servizioaggiuntivo`.`Prezzo` = '" + prezzi_sel.__str__() + "';"
