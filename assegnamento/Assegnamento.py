...
from assegnamento.AggiungiAssegnamento import AggiungiAssegnamento
from assegnamento.ModificaAssegnamento import ModificaAssegnamento


class Assegnamento(QDialog):
    def __init__(self):
        super(Assegnamento, self).__init__()
        loadUi("assegnamento/assegnamento2.ui",self)
        self.tableWidget.setEditTriggers(self.tableWidget.NoEditTriggers)
        self.tableWidget.setHorizontalHeaderLabels(["Cimitero", "Dipendente"])
    ...
    def goto_modifica(self):
        try:
            ...
            self.mod = ModificaAssegnamento(self.selected)
    ...
    def popola_lineEdit(self):
            ...
            self.mod.cimitero.setCurrentText(str(result[self.selected][0]))
            self.mod.dipendente.setCurrentText(str(result[self.selected][1]))
    ...
    def funz_cancella(self):
        try:
            ...
            if reply == QMessageBox.Yes:
                ...
                self.dipendenti = []
                for x in result:
                    self.dipendenti.append(x[1])

                #prendo il codice selezionato
                codiceSel = self.codici[self.selected]
                dip_sel = self.dipendenti[self.selected]

                query2 = "DELETE FROM `assegnamento` WHERE `assegnamento`.`Cimitero` = '" + codiceSel.__str__()+ "' AND `assegnamento`.`Dipendente` = '"+ dip_sel.__str__()+ "';"
