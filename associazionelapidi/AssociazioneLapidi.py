...
from associazionelapidi.AggiungiAssociazioneLapidi import AggiungiAssociazioneLapidi
from associazionelapidi.ModificaAssociazioneLapidi import ModificaAssociazioneLapidi


class AssociazioneLapidi(QDialog):
    def __init__(self):
        super(AssociazioneLapidi, self).__init__()
        loadUi("associazionelapidi/associazionelapidi2.ui",self)
        self.tableWidget.setEditTriggers(self.tableWidget.NoEditTriggers)
        self.tableWidget.setHorizontalHeaderLabels(["Manutenzione","Evento"])
    ...
    def goto_aggiungi(self):
        add = AggiungiAssociazioneLapidi()
    ...
    def goto_modifica(self):
        try:
            ...
            self.mod = ModificaAssociazioneLapidi(self.selected)
    ...
    def popola_lineEdit(self):
            ...
            self.mod.evento.setCurrentText(str(result[self.selected][1]))
    ...
    def funz_cancella(self):
        try:
            ...
            if reply == QMessageBox.Yes:
                ...
                codiceSel = self.eventi[self.selected]
                man_sel = self.manutenzioni[self.selected]

                query2 = "DELETE FROM `associazionelapidi` WHERE `associazionelapidi`.`Manutenzione` = '" + man_sel.__str__()+ "' AND `associazionelapidi`.`Evento` = '"+ codiceSel.__str__()+ "';"
